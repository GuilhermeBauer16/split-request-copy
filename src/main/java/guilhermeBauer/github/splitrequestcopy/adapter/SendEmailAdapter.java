package guilhermeBauer.github.splitrequestcopy.adapter;
...
public class SendEmailAdapter extends HttpAdapter {
    ...
    @Autowired
    public SendEmailAdapter(@Value("${spring.api.send-email}") String url)
    ...
    public void sendNotificationEmail(NotificationEmail notificationEmail){
        try {
            String response = this.restTemplateSimpleWebClient.getRestTemplate()
                    .postForEntity(this.url,notificationEmail, String.class)
                    .getBody();

            log.info(response);
        }catch (RestClientException e){
            log.error(ErrorHandler.EMAIL_SEND);
        }
    }
